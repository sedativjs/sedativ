<img src="https://raw.githubusercontent.com/sedativjs/sedativ-docs/main/src/assets/sedativ.png" alt="Sedativ Logo" width="100%" height="auto" style="visibility: visible; object-fit: cover; max-width: 100%; max-height: 100%"><br>
...
Sedativ requires only **[Deno](https://deno.com/)** (v2.6.0 or newer) and a browser that supports **[Web Components](https://developer.mozilla.org/en-US/docs/Web/API/Web_components)** (from 2020 onward). Aside from Deno itself, it is completely **dependency-free**. It does not use external packages or third-party tools (such as Vite), yet it still provides the capabilities you can expect.
...
`resolveResponse(request, info)`: The main HTTP entrypoint that handles rate limiting, middleware, endpoint execution, and Gzip compression.
...
On the client, it provides a light wrapper around standard Web Components to add reactivity and shifts module resolution entirely to the browser instead of relying on a heavy build pipeline.

On the server, it mirrors client-side API design using standard Response objects, keeping routing and endpoint logic clean while operating close to the Deno runtime. 

Sedativ enforces a **strict separation of concerns** - you always know where code executes just by checking the file extension.

The idea is simple:

***.mjs*** **files run on the server and have direct access to the parsed request** ***req***:
...
***.js*** **files run on the client and have direct access to the component reference** ***ref***:
...
This documentation explains how the framework operates internally, under the hood, line by line. Most frameworks sell heavy abstractions and document surface-level features; here you will learn the architecture from the inside out and understand the underlying mechanics.

This is basically an executable specification, not black-box bloatware with a "convention over comprehension" attitude, so the recommended way to learn Sedativ is to follow the source code `main.mjs` while reading along.
...
Sedativ is a Deno-native framework. If you do not have Deno installed on your machine yet, open terminal and execute the command for your operating system:
...
// main.mjs
export default { 
  fetch: (request, info) => request.url.endsWith('/_reload') && reloadServer ? reloadServer(request) : resolveResponse(request, info) 
}
```
...
Sedativ uses native JavaScript template literals for HTML components and SQL queries. To enable syntax highlighting, install the official **[Sedativ](https://marketplace.visualstudio.com/items?itemName=sedativ.sedativ-vscode)** extension for VS Code or an equivalent plugin for your chosen text editor.
...
flowchart TD
  ...
  E -->|.mjs| F[Run Server Handler]
  E -->|.html| G[Hydrate Layout]
  E -->|mimeText| J[Static Text]
  E -->|mimeBinary| K[Binary Asset]
  F -->|403 Forbidden| Out
...
const root = `file://${Deno.cwd().replaceAll('\\', '/')}/`
const [dbModule, middlewareModule, cacheModule] = await Promise.all(['db.mjs', 'middleware.mjs', 'cache.mjs']
  .map(file => import(new URL(file, root)).then(m => m.default).catch(err => (console.warn('\x1b[33mWarning\x1b[0m:', err.message || err), null))))
...
const LRU = (map) => {
  if (!Object.hasOwn(map, 'get')) {
    map.get = (key) => map.has(key) ? (val => (map.delete(key), Map.prototype.set.call(map, key, val), val))(Map.prototype.get.call(map, key)) : undefined
    map.set = (key, val) => (map.delete(key), Map.prototype.set.call(map, key, val), map.size > config.maxCacheEntries && map.delete(map.keys().next().value), map)
  } 
  return map
}
const caches = {
...
`encoder`: Standard Web API used to convert text strings into raw binary data (Uint8Array).

`root`: A file:// URL pointing to the current project directory `Deno.cwd()`, with path separators normalized for cross-platform compatibility.

`Promise.all(...)`: Loads all optional modules in parallel to keep server startup fast.

`['db.mjs', ...].map(file => import(new URL(file, projectUrl)))`: Dynamic URL resolution anchored to `Deno.cwd()` bypasses Deno LSP static module checks for missing optional files, resolving them relative to the project root instead of the current module.

`module => module.default`: Extracts the default exported function from each loaded file.

`.catch(...)`: Logs loading warnings and returns null as a fallback, allowing request handlers to cleanly bypass missing optional modules.
...
- `isDev`: Evaluates environment variables to determine if the server is running locally. It returns `true` if the `DENO_ENV` variable is not set to `production` and there is no platform deployment identifier `DENO_DEPLOYMENT_ID`.

- `maxRequestsPerSecond`: Limits requests allowed from a single IP address during a one-second window before the server denies access with a `429 Too Many Requests` response.

- `maxCacheEntries`: Limits individual in-memory cache collections to restrict RAM consumption.
...
- `map.get`: Removes and immediately appends an item back to the end of the collection when read, establishing its freshness.
...
`caches.routes.get(candidate)`: Inspects the route cache to see if the file status of the candidate path is already known.
...
const resolveRequest = async (request) => {
  ...
  const body = request.body ? await parsers[type](request) : ''
...
`new URL(url)`: Uses the standard web URL utility to parse the full location string.

`pathname`: Isolates the clean path string (such as `/user/profile`) while stripping away any trailing URL query parameters.

`resolveRoute(pathname)`: Invokes the file-system router using the extracted `pathname` to locate a physical matching asset within the `./src` directory.

`route, params`: Captures the validated disk path string and populates the `params` array with any remaining trailing URL segments left over from the `route` match.
...
`cookies`: Parses the incoming HTTP Cookie header string into a key-value object. It splits cookies using a `;`  spacer, discards empty entries using a `.filter(Boolean)`, ensures each item contains an assignment operator `=`, replaces the first `=` with a hidden null-byte character `(\x00)`, and splits on that null byte. This isolates cookie names from their values safely without using regular expression.

`body`: Checks whether the incoming request contains a body stream. If present, the body is parsed according to its detected content type. If no body is present, an empty string is returned. Parsing errors are propagated to the request handler.
...
`locals`: Generates a fresh, isolated object literal for every request where custom middleware or interceptors can store request-scoped data.

`return { ... }`: Aggregates the HTTP details, filesystem target, structural route array, query variables, cookie records, decoded body, and local object into a single, uniform request context passed to downstream handlers.

## Runtime

```js
const resolveRuntime = async (req, cache, stat) => {
  if (req.ext === 'mjs') {
...
```js
const resolveResource = async (req, cache) => {
  const now = Date.now()
...
`resolveResource`: Serves cached files or constructs HTTP headers, reads disk content, and conditionally caches eligible assets in memory.
...
`stat.size <= cache.maxSize`: Checks if the asset size does not exceed the maximum allowed RAM cache threshold, preventing large files from occupying and exhausting system RAM.
...
const resolveResponse = async (request) => {
  try {
    const now = Date.now()
    const ip = request.headers.get('x-forwarded-for')?.split(',')[0].trim() || info?.remoteAddr?.hostname || '127.0.0.1'
...
`ip`: Extracts the client IP address from proxy headers `'x-forwarded-for'`, falls back to the native connection `info.remoteAddr`, or defaults to local loopback `127.0.0.1`.
...
`resolveRequest(request)`: Normalizes request details and extracts target route metadata. If no route matches `!req.route`, the handler returns a `404 Not Found` response.
...
`new Headers(res.headers)`: Clones the response headers into a mutable Headers instance so they can be safely updated.
...
`gzipHeaders.set('content-encoding', 'gzip')`: Informs the client that the response payload is compressed using Gzip.
...
```js
const resolveReload = (timer, clients = new Set()) => {
  console.log('Live-reload enabled ⚡');
  (async () => {
    for await (const _ of Deno.watchFs('./src')) {
      clearTimeout(timer)
      timer = setTimeout(() => clients.forEach(client => client.readyState === 1 && client.send('reload')), 100)
    }
  })()
...
`resolveReload`: Monitors project files for changes during local development and opens a real-time connection to refresh pages automatically when a file is saved.
...
`client.readyState === 1 && client.send('reload')`: Makes sure the connection is fully open before sending the `'reload'` command.
...
`.mjs`: Run server-side logic and return raw data instead of serving static HTML files.
...
`$(async req => { ... })`: Async before `req` is used when route needs to use `await` (for example, to query databases or make external API calls).
...
`cache.mjs`: Defines caching rules and parameters per route, returning configuration settings back to the framework server.
...
`middleware.mjs`: Intercepts all incoming server traffic and provides a central place to modify request data, log server events, or block unauthorized requests early.
...
`db.mjs`: Executes database initialization logic and attaches query utilities directly to the parsed request object `req`.
...
`const { sql } = req`: Extracts the query utility attached after request processing.
...
`npm:postgres`: Imports the PostgreSQL driver to connect to production databases over the network using Deno's native npm specifier support.

`postgres(Deno.env.get('DATABASE_URL'))`: Initializes a database connection pool using the `DATABASE_URL` environment variable.

`await sql`: Executes table setup using top-level `await` when the module is initialized on server startup.

`export default req => req.sql = sql`: Attaches the PostgreSQL query instance directly to `req.sql` for downstream route handlers to access.
...
In this setup, migrating from SQLite to Postgres requires no endpoint modifications, because the `npm:postgres` driver processes tagged template literals similarly to the custom SQLite wrapper defined earlier.
...
- **Zero Loading Boilerplate**: Progressive element upgrade happen automatically at the browser engine level, eliminating code-splitting wrappers, asset preloaders, or dynamic import boilerplate.
...
`<script type="module" src="/main-layout.js"></script>`: Loads component definition using a native JavaScript module script. Pages can load multiple script modules directly or rely on a single entry point file.
...
`import './read-me.js'`: Imports child component file. Executing a component module registers its custom element tags with the browser globally. Components are executed as side effect imports rather than assigned to named JavaScript variables:
...
```js
let reifier = (value, tag, effects = new Set(), range = document.createRange()) => {
  let remove = () => (
  ...
  const reload = async (ref, strings, values) => {
    const runHook = hook => Promise.all([typeof hook === 'function' ? hook() : hook].flat().map(value => value?.finished ?? value))
    await runHook(ref.exit)
    ref.replaceChildren(range.createContextualFragment(strings.map((string, i) => string + (values[i] ?? '')).join('')))
  ...
  typeof value === 'function' && !tag
    ...
    : tag && !customElements.get(tag) && customElements.define(tag, class extends HTMLElement {
        ...
        html(strings, ...values) { reload(this, strings, values) }
      })
...
- `$(fn, 'tag-name')`: **Web Component** - Registers a native HTML `custom element` tied to a render function.
...
`value`: Holds the initial primitive value, state object, side effect function, or component render function.

`tag`: Optional string used either as a Web Component custom element tag name or a `localStorage` persistence key.

`effects`: A unique list (`Set`) that stores subscriber functions listening to the signal instance.

`range`: A native browser range instance used to transform raw template literal strings into live `DocumentFragment` DOM nodes.
...
`remove`: Clears active dependency subscriptions and resets tracking state upon component or effect teardown.

- `effects.clear()`: Unsubscribes all tracking functions to prevent memory leaks and stop background updates.

- `reifier = null`: Resets the active tracking context.

`render`: Executes rendering/side effects, establishes active tracking context, and registers cleanup handlers.

- `reifier = () => value(ref)`: Assigns a tracking function to reifier during execution so any signal read in getter mode records it as a subscriber.

- `value(ref)`: Invokes the effect or component function, passing the target DOM reference if available.
...
- `reifier = null`: Resets the active tracking context post-execution to prevent accidental tracking outside execution passes.

`reload`: Manages DOM template reconstruction and asynchronous lifecycle transitions during render updates.
...
- `await runHook(ref.exit)`: Awaits exit transitions or animations before DOM modification.

- `strs.map(...).join('')`: Merges template literal strings and dynamic values into a raw HTML string.

- `range.createContextualFragment(...)`: Parses raw HTML strings directly into live DOM `DocumentFragment` nodes.

- `ref.replaceChildren(...)`: Swaps element children with the new fragment in a single DOM update pass.

- `await runHook(ref.enter)`: Awaits entrance transitions or animations after new DOM nodes enter the document.
...
`typeof value === 'function' && !tag`: Identifies standalone reactive side effects (functions without an assigned tag name).

`(render(), remove())`: Immediately runs the effect to collect initial dependencies, then invokes cleanup setup.
...
`localStorage.setItem(...)`: Serializes and writes default state to `localStorage` if no key exists.
...
- `effects.add(reifier)`: If an effect or component is currently running, adds it to effects as a subscriber.

- `value`: Returns the current signal value.

`newValue !== undefined`: **Setter mode**

- `value = newValue`: Updates internal state to `newValue`.

- `localStorage.setItem(...)`: Updates serialized data in `localStorage` if persistence key `tag` is present.
...
- `effects.forEach(fn => fn())`: Executes each registered subscriber function once microtasks flush.
...
`ref.querySelector('button')`: Queries elements inside the component's subtree. Because the search is scoped directly to `ref`, global IDs or special framework refs are not needed to access child DOM nodes.
...
`count()`: Reading the value with no arguments registers the active tracking context (such as a side effect callback) as a subscriber.
...
⚠️ This built-in behavior completely eliminates the need for complex external global state management libraries that other frameworks may force you to adopt.
...
`let counter = $(0)`: Declares state inside the component function scope, creating an isolated state instance for each rendered custom element.
...
`$(() => doubled(count() * 2))`: Computes derived values using standard reactive side effects. When the base signal `count()` updates, it triggers its direct effect, which updates `doubled()`, cascading state updates cleanly before the microtask queue executes the final layout updates.
...
`$('dark', 'app-user-theme')`: Passing a string key as the second argument enables browser storage persistence. Upon initialization, the engine reads `localStorage` for the key. If found, it hydrates the signal with the saved JSON value; otherwise, it falls back to the initial default value `'dark'` and writes it to storage.

`theme(...)`: Updating a persistent signal automatically serializes the new value to `localStorage` under the registered key, ensuring state persists across page reloads.
...
`$(ref => { ... }, 'local-time')`: The component setup code runs immediately when the element renders in the DOM (driven by the native `connectedCallback()`).

`return () => { ... }`: Returning a cleanup function from the component setup code registers teardown logic. The engine automatically calls this function when the element is removed from the DOM (driven by the native `disconnectedCallback()`), safely clearing timers, intervals, or event listeners.
...
`async (ref) => { ... }`: Defines an asynchronous component. Code up to the first `await` keyword executes synchronously, rendering initial loading states and setting up signal subscribers immediately on initial render.
...
`return () => ...`: Async components support cleanup returns natively. The engine awaits the setup promise, extracts the returned cleanup function, and registers it to run when the component is removed from the DOM.
...
`ref.getAttribute('card-title')`: Reads plain-text string attributes directly from the child component element during setup.
...
`ref.dispatchEvent(...)`: Dispatches custom events directly from the child component element (ref) to send signals or payloads upward through standard DOM event propagation.
...
`ref.onchange`: Captures form input changes globally on the component via event delegation, updating signals only when user blur input or select option to avoid re-rendering on every keystroke.

`$(() => ref.html...)`: Wraps rendering in a side effect to re-render DOM nodes whenever local signals update.
...
`$(() => ref.html...)`: Re-evaluates the template whenever `show()` changes, rendering or removing `.item` elements dynamically.

`ref.onclick`: Toggles the `show()` signal, triggering a reactive update of the layout.

`ref.enter`: Runs immediately after elements render in the DOM, executing entrance animations on the new nodes.

`ref.exit`: Runs right before elements are removed, pausing DOM cleanup until all returned animations complete.

`fade(direction)`: Queries target elements and returns an array of animation instances for the engine to await.

`item.animate(...)`: Demonstrates native Web Animations API usage, though any helper returning a Promise works identically.

⚠️ Lifecycle hooks automatically await any returned Promise or array of Promises. This allows external animation tools like GSAP to work out of the box without wrappers or framework-specific plugins.
...
`$(() => ref.html...)`: Re-evaluates controls whenever `isPlaying()` or `timestamp()` changes, refreshing button icons and time displays without re-rendering surrounding elements.

`ref.onclick`: Toggles the `isPlaying()` state and calls native media methods (`play()` / `pause()`) directly on the shared element.

`export const video = $(null)`: Stores a live DOM element reference inside an exported signal, allowing other components to control playback.

`ref.enter = () => { ... }`: Captures the `<video>` element after it renders in the DOM, safely setting up listeners without interfering with initial setup.

`video().ontimeupdate`: Listens to native playback events, updating the `timestamp()` signal continuously as the video plays.

⚠️ Always capture DOM elements and assign them to signals inside `ref.enter`. Updating a signal directly in the main component body triggers an immediate re-render before rendering finishes, creating an infinite loop that repeatedly destroys and recreates the video node.
...
const router = () => {
  ...
  $(ref => {
    ...
    const getHref = (event, href = event.target.closest('a')?.getAttribute('href')) => 
    ...
    globalThis.onmouseover = async event => {
      const href = getHref(event)
      const route = href && (await resolveRoute(href)).route
...
`router` intercepts page navigation, verifies file availability over the network, and updates the view.

- `Hybrid Architecture`: Allows server-rendered multi-page apps (MPA) and client-driven single-page apps (SPA) to coexist in the same codebase without global setup.
...
`caches.modules.has(route)`: Checks the cache to reuse the active or resolved fetch Promise, avoiding duplicate HTTP HEAD queries for the same path.
...
`caches.routes.has(route)`: Checks the cache to reuse the registered component or import Promise, preventing redundant module imports for the same path.
...
`if (typeof pathname !== 'string') return`: Guards against non-string inputs to prevent runtime errors.
...
`href?.startsWith('/') && !href.startsWith('//')`: Validates that the link is a root-relative internal path, preserving native browser navigation for external and protocol-relative URLs.
...
While the server automatically injects the reactivity engine into HTML responses, reifier and router can be imported manually. This lets you host static sites without a Deno server (for example, on platforms like GitHub Pages or Cloudflare Pages), or integrate with any custom back end.

Additionally, the reactivity engine can be imported with zero setup or extra dependencies, allowing you to progressively replace any front-end framework or use it solely for global state management.
...
`<main-layout></main-layout>`: Renders the main layout Web Component placeholder inside the document body.
...
`signoff.js`: Runs permanent account deletion automatically upon rendering to the DOM.
...
`signout.js`: Runs session signout routines immediately upon rendering to the DOM.
